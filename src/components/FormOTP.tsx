...
import { apiVerifyOTP } from '../services/authServicer';
...
const FormOTP: React.FC<FormOTPProps> = ({ role, email }) => {
   ...
   const handleOTP = async () => {
      try {
         ...
         await apiVerifyOTP({ email, role, otp: formatOTP });
         if (role === 'customer') {
            Swal.fire({
            ...
            Swal.fire({
               title: "Đăng ký thành công!",
               text: 'Chúng tôi sẽ phản hồi việc đăng ký qua email cho bạn, vui lòng đợi!',
               icon: "success"
            });
         }
      } catch (error: any) {
